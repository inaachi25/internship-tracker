          ...
          return (
            <div key={day} className="relative">
              <button onClick={() => handleClick(ds)} title={holiday?.name}
                ...
                {badge && log && <span className="text-[9px] font-bold text-purple-500 leading-none mt-0.5">{getTotalHours(log)}h</span>}
                {schedH && <span className="text-[9px] font-bold text-indigo-400 leading-none mt-0.5">{getTotalHours(autoMap.get(ds)!)}h</span>}
                ...
                {badge && <span className="absolute -top-1.5 -right-1.5 bg-indigo-600 text-white text-[9px] rounded-full w-4 h-4 flex items-center justify-center font-bold shadow">{getTotalHours(log!)}</span>}
              </button>
              {isConf && (
